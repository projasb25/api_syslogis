...
class MainRepository
{
    ...
    public function getInretailDataToCollect($orgid)
    {
        return DB::table('integration_data as id')
            ...
            ->join('integration_data_detail as idd','idd.id_integration_data','=','id.id_integration_data')
            ->where('id.id_organization', $orgid)
            ->where('id.status', 'PENDIENTE')
            ->get();
    }

    public function getInretailDataToDeliver($orgid)
    {
    ...
    public function InRetail_getDistinctOrgids()
    {
        return DB::table('integration_data as id')
        ...
        try {
            ...
            foreach ($data as $key => &$value) {
                DB::table('massive_load_details')->insert([
                    ...
                    'client_dni' => ($type === 'Logistica inversa') ? $value->delivery_client_dni : $value->collect_client_dni,
                    'client_name' => ($type === 'Logistica inversa') ? $value->delivery_client_name : $value->collect_client_name,
                    'client_phone1' => ($type === 'Logistica inversa') ? $value->delivery_client_phone1 : $value->collect_client_phone1,
                    'client_phone2' => ($type === 'Logistica inversa') ? $value->delivery_client_phone2 : $value->collect_client_phone2,
                    'client_phone3' => ($type === 'Logistica inversa') ? $value->delivery_client_phone3 : $value->collect_client_phone3,
                    'client_email' => ($type === 'Logistica inversa') ? $value->delivery_contact_email : $value->collect_client_email,
                    'client_address' => ($type === 'Logistica inversa') ? $value->delivery_address : $value->collect_address,
                    'client_address_reference' => ($type === 'Logistica inversa') ? $value->delivery_address_reference : $value->collect_address_reference,
                    'ubigeo' => ($type === 'Logistica inversa') ? $value->delivery_ubigeo : $value->collect_ubigeo,
                    'department' => ($type === 'Logistica inversa') ? $value->delivery_department : $value->collect_department,
                    'district' => ($type === 'Logistica inversa') ? $value->delivery_district : $value->collect_district,
                    'province' => ($type === 'Logistica inversa') ? $value->delivery_province : $value->collect_province,
    ...
    public function insertMassiveLoadIntegration($data, $orgname, $type)
    {
        ...
        try {
            ...
            foreach ($data as $key => &$value) {
                ...
                DB::table('load_integration_detail')->where('id_load_integration',$value->id_load_integration)->update(['status'=>'PROCESADO']);
            }
            DB::commit();
        } catch (\Exception $e) {
            DB::rollback();
        ...
        try {
            ...
            foreach ($data as $key => &$value) {
                DB::table('massive_load_details')->insert([
                    ...
                    // 'client_date2' => $value['client_date2'] ?? null,
                    'client_barcode' => $value->client_barcode,
                    'client_dni' => ($type === 'Logistica inversa') ? $value->collect_client_dni : $value->delivery_client_dni,
                    'client_name' => ($type === 'Logistica inversa') ? $value->collect_client_name : $value->delivery_client_name,
                    'client_phone1' => ($type === 'Logistica inversa') ? $value->collect_client_phone1 : $value->delivery_client_phone1,
                    'client_phone2' => ($type === 'Logistica inversa') ? $value->collect_client_phone2 : $value->delivery_client_phone2,
                    'client_phone3' => ($type === 'Logistica inversa') ? $value->collect_client_phone3 : $value->delivery_client_phone3,
                    'client_email' => ($type === 'Logistica inversa') ? $value->collect_client_email : $value->delivery_contact_email,
                    'client_address' => ($type === 'Logistica inversa') ? $value->collect_address : $value->delivery_address,
                    'client_address_reference' => ($type === 'Logistica inversa') ? $value->collect_address_reference : $value->delivery_address_reference,
                    // 'coord_latitude' => $value['coord_latitude'] ?? null,
                    // 'coord_longitude' => $value['coord_longitude'] ?? null,
                    'ubigeo' => ($type === 'Logistica inversa') ? $value->collect_ubigeo : $value->delivery_ubigeo,
                    'department' => ($type === 'Logistica inversa') ? $value->collect_department : $value->delivery_department,
                    'district' => ($type === 'Logistica inversa') ? $value->collect_district : $value->delivery_district,
                    'province' => ($type === 'Logistica inversa') ? $value->collect_province : $value->delivery_province,
                    'sku_code' => $value->sku_code,
                    'sku_description' => $value->sku_description,
                    'sku_weight' =>  $value->sku_weight,
                    'sku_pieces' =>  $value->sku_pieces,
                    ...
                    'contact_name' => ($type === 'Logistica inversa') ? $value->collect_contact_name : $value->delivery_contact_name,
                    // 'contact_phone' => $value['contact_phone'] ?? null,
                    // 'payment_method' => $value['payment_method'] ?? null,
                    // 'amount' => $value['amount'] ?? null,
                    // 'collect_time_range' => $value['collect_time_range'] ?? null,
        ...
        try {
            ...
            foreach ($data as $key => &$value) {
                ...
                if ($organization == 122 || $organization = 100) { # Logistica inversa
                    ...
                    $client_info->client_phone2 = $value->delivery_client_phone2;
                    $client_info->client_phone3 = $value->delivery_client_phone3;
                    $client_info->client_email = $value->delivery_contact_email;
                    $client_info->client_address = $value->delivery_address;
                    $client_info->client_address_reference = $value->delivery_address_reference;
                    $client_info->ubigeo = $value->delivery_ubigeo;
                    $client_info->department = $value->delivery_department;
                    $client_info->district = $value->delivery_district;
                    $client_info->province = $value->delivery_province;
                } else {
                    ...
                    $client_info->department = $value->collect_department;
                    $client_info->district = $value->collect_district;
                    $client_info->province = $value->collect_province;
                }
                ...
                DB::table('massive_load_details')->insert([
                    ...
                    'client_date' => date('Y-m-d H:i:s', time() + 86400),
                    'client_barcode' => $value->client_barcode,
                    'client_dni' => $client_info->client_dni,
                    'client_name' => $client_info->client_name,
                    'client_phone1' => $client_info->client_phone1,
                    'client_phone2' => $client_info->client_phone2,
                    'client_phone3' => $client_info->client_phone3,
                    'client_email' => $client_info->client_email,
                    'client_address' => $client_info->client_address,
                    'client_address_reference' => $client_info->client_address_reference,
                    'ubigeo' => $client_info->ubigeo,
                    'department' => $client_info->department,
                    'district' => $client_info->district,
                    'province' => $client_info->province,
                    'sku_code' => $value->sku_code,
                    'sku_description' => $value->sku_description,
                    'sku_weight' =>  $value->sku_weight,
                    'sku_pieces' =>  $value->sku_pieces,
